...
### 魔术光标
添加，或是移除标注，目前支持*坐标及颜色标注*和*区域标注*两种。
...
- `⌥` + **右键轻按**（*Force Touch*：**双指单击**）标注：显示菜单，包含当前光标位置**所有层级下**层叠的*区域标注*，选择一项以移除标注。
- **按住** `⌃`：暂时切换到*选择箭头*。
...
- **轻按**（*Force Touch*：**单击**）：自当前光标位置缩小到上一缩放级别。
- **按住** `⌃`：暂时切换到 *魔术光标*。
- **按住** `⌥`：暂时切换到 *放大镜*。
...
查看 `ExampleTemplates/README.md` 可以了解更多细节。
