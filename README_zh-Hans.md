...
- `⇧` + **轻按** (*Force Touch*: **单击**) 标注: 选择当前位置下的*坐标和颜色标注*, 或**所有层级下**的*区域标注*, 同时保留之前的选择项.
...
- `⌘[`: 如果已选中的*标注*是当前光标位置下所有层级中唯一选中的*标注*, 则切换选中状态到当前光标位置下**比它面积小的**前一个*标注*
- `⌘]`: 如果已选中的*标注*是当前光标位置下所有层级中唯一选中的*标注*, 则切换选中状态到当前光标位置下**比它面积大的**后一个*标注*
...
### 导出
- `⌘E`: 使用当前*模板*导出所有项.
- `⌥⌘E` 或点击右下角列表按钮显示导出选项菜单:
  - `⌥⌘` + `1234567890`: 快速切换当前*模版*.
  - `⌥⌘R` *重新加载所有模板*: 修改模板文件后, 使用该操作以重载所有*模板*文件, 立即生效.
