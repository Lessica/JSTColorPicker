...
public class JSTColorPanel: NSColorPicker, NSColorPickingCustom {
    ...
    static var shared: JSTColorPanel!
    ...
    public func provideNewView(_ initialRequest: Bool) -> NSView {
        if (initialRequest) {
            ...
            JSTColorPanel.shared = self
            
            let pickerNibName = "JSTColorPanel"
    ...
    public func setColor(_ newColor: NSColor) {
        ...
        if (!firstColorChange) {
            ...
            if self.undoManager == nil {
                self.undoManager = ColorUndoManager(colorPanel: self, initialColor: hsv)
            }
    ...
    private let bundle = Bundle(for: JSTColorPanel.self)
    ...
    public override var buttonToolTip: String {
        get {
            return NSLocalizedString("JSTColorPicker", comment: "JSTColorPanel")
        }
    }
...
extension JSTColorPanel: ChangeColorDelegate {
