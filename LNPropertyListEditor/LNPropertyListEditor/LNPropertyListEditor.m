...
{
    return _outlineView.sortDescriptors;
}

- (void)setSortDescriptors:(NSArray <NSSortDescriptor *> *)sortDescriptors
{
    _outlineView.sortDescriptors = sortDescriptors;
}

- (BOOL)isSortByClickingHeaderEnabled
{
    return _keyColumn.sortDescriptorPrototype != nil || _typeColumn.sortDescriptorPrototype != nil || _valueColumn.sortDescriptorPrototype != nil;
}

- (void)setSortByClickingHeaderEnabled:(BOOL)sortByClickingHeaderEnabled
{
    if (sortByClickingHeaderEnabled) {
        _keyColumn.sortDescriptorPrototype = [NSSortDescriptor sortDescriptorWithKey:@"key" ascending:YES selector:@selector(localizedCaseInsensitiveCompare:)];
        _typeColumn.sortDescriptorPrototype = [NSSortDescriptor sortDescriptorWithKey:@"type" ascending:YES selector:@selector(compare:)];
        _valueColumn.sortDescriptorPrototype = nil;
    } else {
        _keyColumn.sortDescriptorPrototype = nil;
        _typeColumn.sortDescriptorPrototype = nil;
        _valueColumn.sortDescriptorPrototype = nil;
    }
}
...
- (NSOutlineView *)embeddedOutlineView {
    return _outlineView;
}
...
- (BOOL)validateMenuItem:(NSMenuItem *)menuItem
{
    ...
    if (menuItem.action == @selector(copyValue:))
    {
        return !_copyValueOnly;
    }
...
- (IBAction)copyValue:(id)sender
{
    ...
    [NSPasteboard.generalPasteboard setString:[NSString stringWithFormat:@"%@", node.value] forType:NSPasteboardTypeString];
}
